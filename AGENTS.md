...
- **Rule compiler** (`src/compile/`) — pure-data IR plus a lowering pass and two
  tagged-template front-ends. Rules can be written either with the per-clause DSL
  (`rule(name, arity)(clause\`...\`)`from`yopl/compile/clause`) or with the strict-Prolog
whole-program parser (`prolog\`...\``from`yopl/compile/prolog`); both emit identical IR
for the shared subset. The IR has 5 Term kinds + 4 Goal kinds. Lowering substitutes
activation-fresh logic Variables; the **`Lit`-walker** descends into plain objects and
arrays inside a literal's value, so a template like `Lit({age: Var('A')})`doubles as a
pattern matcher and a constructor. The public barrel`yopl/compile`re-exports IR
constructors + lowering + validation +`open`/`soft`/`\_`/`any`from`deep6`for
fine-grained match control. Front-ends emit IR; the runtime never sees IR. Design:`dev-docs/compiler-ir.md`.

...
| `bench-inline-goals-bridge.js` | `bench-inline-goals.js` via bridge helpers + `sumList50_math`/`_bridge`       | Workstream-2 tier-1/3 POC: `reversible3` (single-walk deref) vs the baseline native; the in-file sum pair stresses depth. Verdict: perf-neutral — see runtime-protocols.md § Tier 1 + 3 results.              |
